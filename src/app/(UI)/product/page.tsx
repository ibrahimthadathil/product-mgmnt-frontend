...
//       const data:any = await addMutation.mutateAsync(newProduct);
...
//     try {
//       setIsModalOpen(false);
//       const data:any = await editMutation.mutateAsync({
...
//     try {
//       const data:any = await deleteMutation.mutateAsync(id);
//       toast.success(data.message);
//     } catch (error) {
...
import { UseRQ } from "@/hooks/useRQ";
import { UseRMutation } from "@/hooks/useMutation"; // your reusable mutation hook
import { DataTable } from "@/components/table/dataTable";
import { ColumnDef } from "@tanstack/react-table";
import Image from "next/image";
import { toast } from "sonner";
...
export default function ProductsPage() {
  ...
  // -----------------------------------------------------------------------
  // MUTATIONS -> use the reusable UseRMutation with onSuccess callbacks
  // -----------------------------------------------------------------------

  // ADD mutation: centralized success handling via fourth parameter
  const addMutation = UseRMutation(
    ...
    "products", // invalidation key -> will invalidate queries with key 'products' on settle
    (res) => {
      // onSuccess for add
      // res is { data: resultFromApi }
  ...
  // -----------------------------------------------------------------------
  // HANDLERS -> now much simpler. they only trigger mutate(...)
  // -----------------------------------------------------------------------

  // BEFORE: had try/catch, mutateAsync and manual toasts
  // AFTER: simple mutate call — success / error handled inside UseRMutation

  const handleAddProduct = (newProduct: FormData) => {
    ...
    <main className="max-h-screen bg-background">
      <div className="w-full max-w-7xl mx-auto px-4 py-8">
        ...
        <ProductModal
          isOpen={isModalOpen}
          onClose={handleCloseModal}
          // pass correct handler depending on whether editingProduct exists
          onSubmit={editingProduct ? handleEditProduct : handleAddProduct}
          initialProduct={editingProduct}
        />
